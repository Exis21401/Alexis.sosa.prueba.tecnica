...
namespace KartGame.Track
{
    ...
    public class TrackManager : MonoBehaviour
    {
		...
		// Creo una referencia para el panel de Game Over
		public GameObject GameOverPanel;
		public float BestTime; //Creo una variable para almacenar la mejor vuelta
        ...
        void RacerHitCorrectCheckpoint (IRacer racer, Checkpoint checkpoint)
        {
            if (checkpoint.isStartFinishLine)
            {
                ...
                if (CanEndRace ())
                    StopRace ();
				if (racerCurrentLap >= 3) { GameOverPanel.SetActive(true); } //Muestro el panel de Game Over al terminar la carrera
